package io.github.nginx.ops.client.util;
...
import io.github.nginx.ops.client.domain.dto.ConfInfoCommDTO;
import io.github.nginx.ops.client.domain.dto.ConfInfoServerDTO;
import io.github.nginx.ops.client.domain.dto.ConfInfoServerItemDTO;
...
public class NginxConfUtils {
  ...
  public static NgxBlock buildBlockServer(ConfInfoServerDTO dto) {
    ...
    // 额外参数
    if (ObjectUtil.isNotEmpty(dto.getConfInfoCommDtoList())) {
      dto.getConfInfoCommDtoList().forEach(item -> setSameParam(item, ngxBlockServer));
    }
    // location参数配置
    if (ObjectUtil.isNotEmpty(dto.getConfInfoServerItemDTOList())) {
      for (ConfInfoServerItemDTO item : dto.getConfInfoServerItemDTOList()) {
